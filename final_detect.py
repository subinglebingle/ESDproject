...
prev_time = 0
last_email_time = 0
EMAIL_INTERVAL = 60  # 60초마다 최대 1회 메일 발송 제한

def send_email():
    try:
        smtp = smtplib.SMTP('smtp.gmail.com', 587)
        smtp.starttls()
        smtp.login('embeddedsystemdesign12@gmail.com', 'vfnfiybfutnwsbba')  # 본인 이메일/비밀번호로 변경
        msg = MIMEText('room no.10')
        msg['Subject'] = '경고: drinks detected'
        msg['From'] = 'embeddedsystemdesign12@gmail.com'
        msg['To'] = 'embeddedsystemdesign@naver.com'  # 수신자 이메일로 변경
...
try:
    while True:
            ...
            if pred is not None:
                for *box, conf, cls in pred:
                    ...
                    if label == "drinks":
                        ...
                        gx1 = int(bx1 * scale_roi_x) + x1
                        gx2 = int(bx2 * scale_roi_x) + x1
                        gy1 = int(by1 * scale_roi_y) + y1
                        gy2 = int(by2 * scale_roi_y) + y1
        ...
        if drink_detected:
           ...
           print(curr_email_time, last_email_time, EMAIL_INTERVAL)
